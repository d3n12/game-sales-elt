...
def _normalize_title(title: str) -> str:
    ...
    title = title.replace(" :", ":")                             # remove space before colon
    title = re.sub(r"(pikachu!)\s+(pok)", r"\1 / \2", title)     # insert missing slash in Pok\u00e9mon: Let's Go bundle
    if re.match(r"^pok[e\u00e9]mon\b", title):                   # strip " Version" anywhere in Pok\u00e9mon titles
        title = re.sub(r"\s+version\b", "", title)               # e. g. "HeartGold Version / SoulSilver Version" \u2192 "HeartGold / SoulSilver"
