...
def transform_to_silver(conn: duckdb.DuckDBPyConnection) -> int:
    ...
    def to_units(series: pd.Series) -> pd.Series:
        return (
    ...
    df["game_title"]           = df["Game Title"].apply(normalize_title)
    df["platform_name"]        = df["system"]
    df["fiscal_year"]          = df["Fiscal Year"].apply(normalize_fiscal_year)
    df["snapshot_date"]        = pd.to_datetime(df["as of"], errors="coerce")
    df["global_sales"]         = to_units(df["Global"])
    df["japan_sales"]          = to_units(df["Japan"])
    df["outside_japan_sales"]  = to_units(df["Outside of Japan"])
    df["ltd_global_sales"]     = to_units(df["Life-to-date Global"])
